...
<div>
	<nav>
	  <ul class="pagination">
	    ...
	    <li class="active"><a href="#">月</a></li>
	    <li><a href="<%=basePath%>admin/analysis_salesByYear">年</a></li>
	  </ul>
	</nav>
</div>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$.post({
			url:"<%= path %>/admin/data_getSalesByMonth",
			success:function(data){
				// console.log(data);
				
				var p = 0;										// make date consistant
				var objData = [];
				var tDate = new Date(data.date[0]);
				while( p < data.date.length ) {
					 // console.log(tDate.toDateString());
					if( tDate.getTime() == Date.parse(data.date[p]) ) {
						objData.push({
							"date" : data.date[p],
						...
						var t = tDate.getFullYear() + "-" + (tDate.getMonth()+1);
						objData.push({
							"date" : t,
							"sales": 0
						});
				...
				var xparseTime = d3.time.format("%Y-%b");
